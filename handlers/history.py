...
async def show_history_today(callback: CallbackQuery, habit_service: HabitService):
    today = datetime.utcnow()
    await _render_history(callback, today, habit_service)
...
async def _render_history(callback: CallbackQuery, target_date: datetime, habit_service: HabitService):
    date_str = target_date.strftime("%Y-%m-%d")
    logs = await habit_service.get_daily_logs(callback.from_user.id, date_str)
    habits = await habit_service.get_user_habits(callback.from_user.id, active_only=False)
