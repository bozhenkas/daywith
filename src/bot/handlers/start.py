...
async def cmd_start(message: Message, state: FSMContext, habit_service: HabitService):
    ...
    if user.get("digest_time") and user.get("digest_time") != "21:00":
...
async def on_time_input(message: Message, state: FSMContext, habit_service: HabitService):
    if not validate_time_format(message.text or ""):
...
async def on_main_menu(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        pass
    await callback.message.answer(get_msg("statistics.menu_header"), reply_markup=get_main_menu_keyboard())
