...
async def show_habits_callback(callback: CallbackQuery, habit_service: HabitService):
...
async def process_habit_goal(callback: CallbackQuery, state: FSMContext, habit_service: HabitService):
    await callback.answer()
...
async def start_rename_habit(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    h_id = callback.data.split(":")[2]
...
async def process_rename_habit(message: Message, state: FSMContext, habit_service: HabitService):
    if not message.text:
        return
    data = await state.get_data()
